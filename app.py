...
if not df.empty:
    ...
    # Baru: Conversion Rate (CR)
    filtered_df['CR IGD to Ipt'] = (filtered_df['Total IGD to Ipt'] / filtered_df['Total IGD']).replace([np.inf, -np.inf], 0).fillna(0) * 100
    ...
    # FUNGSI UNIVERSAL STACKED CHART
    def create_stacked_chart(df_data, title, col_top, col_bottom, col_total, col_growth_name, y_label, is_revenue=False, line_col=None):
        with st.container(border=True):
            ...
            for i, cabang in enumerate(selected_cabang):
                ...
                # Stacked Bar: Bottom (Non JKN / Opt)
                ...
                # Baru: Line Chart untuk Conversion Rate (jika ada)
                if line_col and line_col in branch_df:
                    fig.add_trace(go.Scatter(
                        x=branch_df['Bulan'], y=branch_df[line_col], name=f"CR {cabang}",
                        mode='lines+markers+text',
                        line=dict(color=colors.get(cabang)['dark'], width=2),
                        text=branch_df[line_col].apply(lambda x: f"{x:.1f}%"),
                        textposition="top center",
                        yaxis="y2", hovertemplate=f"<b>CR {cabang}</b>: %{{y:.2f}}%<extra></extra>"
                    ))

            # Layout adjustment for dual axis (jika ada line chart)
            layout_args = dict(
                barmode='group', height=500, margin=dict(t=80, b=10),
                yaxis=dict(title=y_label, range=[0, df_data[col_total].max() * 1.6]),
                legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
            )
            if line_col:
                layout_args['yaxis2'] = dict(title="Conversion Rate (%)", overlaying="y", side="right", range=[0, 120], showgrid=False)
    ...
    # --- EKSEKUSI GRAFIK ---
    create_stacked_chart(filtered_df, "📈 Realisasi Revenue (Opt vs Ipt)", 
    ...
    # BARU: Analisis Konversi IGD ke Ipt (Bar + Line)
    create_stacked_chart(filtered_df, "🎯 Konversi IGD ke Rawat Inap (Ipt)", 
                         'Volume IGD to Ipt JKN', 'Volume IGD to Ipt Non JKN', 'Total IGD to Ipt', 
                         'Total IGD to Ipt_Growth', "Volume IGD to Ipt", line_col='CR IGD to Ipt')
