...
if not df.empty:
    ...
    def create_stacked_chart(df_data, title, col_jkn, col_nonjkn, col_total, col_growth_name, y_label):
        with st.container(border=True):
            ...
            for i, cabang in enumerate(selected_cabang):
                ...
                # Trace 1: Non JKN (Bawah) - Menampilkan Nilai di Bar
                ...
                # Trace 2: JKN (Atas) - Menampilkan Nilai di Bar
                fig.add_trace(go.Bar(
                ...
                fig.add_trace(go.Bar(
                    ...
                    textfont=dict(color=g_colors, size=11),
                    marker_color='rgba(0,0,0,0)', 
                    hoverinfo='skip' # Agar tidak menghalangi hover bar di bawahnya
                ))

            fig.update_layout(barmode='group', height=450, margin=dict(t=80, b=10),
                              yaxis_title=y_label,
                              yaxis=dict(range=[0, df_data[col_total].max() * 1.45]),
    ...
    with st.container(border=True):
        ...
        fig_rev.update_layout(barmode='group', height=400, margin=dict(t=80, b=10),
                              yaxis=dict(range=[0, filtered_df['Actual Revenue (Total)'].max() * 1.45]))
        st.plotly_chart(fig_rev, use_container_width=True)

    # 2. Volume OPT, Ranap, & IGD
    create_stacked_chart(filtered_df, "👥 Volume Outpatient (OPT)", 'Volume OPT JKN', 'Volume OPT Non JKN', 'Total OPT', 'Total OPT_Growth', "Jumlah Pasien")
    create_stacked_chart(filtered_df, "🏥 Volume Inpatient (Ranap)", 'Volume IPT JKN', 'Volume IPT Non JKN', 'Total IPT', 'Total IPT_Growth', "Jumlah Pasien")
    create_stacked_chart(filtered_df, "🚑 Volume IGD", 'Volume IGD JKN', 'Volume IGD Non JKN', 'Total IGD', 'Total IGD_Growth', "Jumlah Pasien")
