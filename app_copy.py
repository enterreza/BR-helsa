...
SHEET_NAME = 'app_data'import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
...
if not df.empty:
    ...
    st.title("📊 Dashboard Performa Operasional Helsa-BR 2025")
    ...
    # FUNGSI HELPER UNTUK MEMBUAT STACKED CHART
    def create_stacked_chart(df_data, title, col_jkn, col_nonjkn, col_total, col_growth, y_label):
        with st.container(border=True):
            ...
            for i, cabang in enumerate(selected_cabang):
                ...
                branch_df['Total_Hover'] = branch_df[col_total]
                ...
                fig.add_trace(go.Bar(
                    ...
                    hovertemplate=f"<b>{cabang} (Non JKN)</b>: %{{y:,}}<br>Total: %{{customdata:,}}<extra></extra>"
                ))
                # JKN (Dark)
                fig.add_trace(go.Bar(
                    ...
                    hovertemplate=f"<b>{cabang} (JKN)</b>: %{{y:,}}<br>Total: %{{customdata:,}}<extra></extra>"
                ))
                # Growth Label
                fig.add_trace(go.Bar(
